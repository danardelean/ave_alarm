pag_table_new["widget_set_device_magdet"] = {
	onload: function()
	{
		...
		storing = false;
		//
		item_room(".widget_set_device_magdet", this.xml_any);
		//
		...
		global_item_init(".widget_set_device_magdet", "name", "input", null, null, null, nameDev, this.xml_any.children("name").attr("maxlength"));
	...
	par_save: function()
	{
		ena_save = true;
		//
		save_item_name(".widget_set_device_magdet", this);
		//
		this.xml_any.find("room").text(id_room_selected);
		//
		if (this.son1 != null)
		{
			this.son1.children("name").text($(".widget_set_device_magdet .name").val() + "a");
			this.son1.find("room").text(id_room_selected);
		}
		if (this.son2 != null)
		{
			this.son2.children("name").text($(".widget_set_device_magdet .name").val() + "b");
			this.son2.find("room").text(id_room_selected);
		}
		//
		var area_str = save_item_area("#area_container_set", this);
		if (this.son1 != null)
			this.son1.find("area_alarm").text(area_str);
		if (this.son2 != null)
			this.son2.find("area_alarm").text(area_str);
	},
