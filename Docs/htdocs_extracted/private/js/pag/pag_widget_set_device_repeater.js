pag_table_new["widget_set_device_repeater"] = {
	onload: function()
	{
		...
		storing = false;
		//
		item_room(".widget_set_device_repeater", this.xml_any);
		//
		...
		global_item_init(".widget_set_device_repeater", "name", "input", null, null, null, nameDev, this.xml_any.children("name").attr("maxlength"));
		...
		if (conf.attr("type") == "MENU")
		{
			if (conf.children("act").text() == "SAVE")
			{
				if (conf.children("res").text() == "SAVED")
				{
					...
					// attenzione il file di configurazione serve solo per poter ricevere le indication del nuovo repeater appena peirato
					// non richiedere durante la formazione della lista di add_other_devices
					file_conf_request();
				}
	...
	//
	name: "widget_set_device_repeater",
	...
	par_save: function()
	{
		ena_save = true;
		//
		save_item_name(".widget_set_device_repeater", this);
		//save_room
