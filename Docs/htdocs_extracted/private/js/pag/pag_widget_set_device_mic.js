pag_table_new["widget_set_device_mic"] = {
	onload: function()
	{
		...
		storing = false;
		//
		item_room(".widget_set_device_mic", this.xml_any);
		//
		...
		global_item_init(".widget_set_device_mic", "name", "input", null, null, null, nameDev, this.xml_any.children("name").attr("maxlength"));
	...
	par_save: function()
	{
		ena_save = true;
		//
		save_item_name(".widget_set_device_mic", this);
		//
		this.xml_any.find("room").text(id_room_selected);
		//
		if (this.son1 != null)
		{
			this.son1.children("name").text($(".widget_set_device_mic .name").val() + "a");
